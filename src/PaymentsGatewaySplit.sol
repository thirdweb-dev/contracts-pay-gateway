...
contract PaymentsGatewaySplit is EIP712, Ownable, ReentrancyGuard {
    ...
    function _distributeFees(address tokenAddress, address feeRecipient, uint256 fee) private {
        if (_isTokenNative(tokenAddress)) {
            SafeTransferLib.safeTransferETH(feeRecipient, fee);
        } else {
            SafeTransferLib.safeTransferFrom(tokenAddress, msg.sender, feeRecipient, fee);
        }
        emit FeePayout(msg.sender, feeRecipient, tokenAddress, fee);
    }
    ...
    function startTransfer(PayRequest calldata req, bytes calldata signature) external payable nonReentrant {
        ...
        if (req.fee > req.tokenAmount) {
            revert PaymentsGatewayMismatchedValue(req.fee, req.tokenAmount);
        }
