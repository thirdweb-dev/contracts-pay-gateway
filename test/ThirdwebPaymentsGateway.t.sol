...
contract ThirdwebPaymentsGatewayTest is Test {
    ...
    function _prepareAndSignData(
        ...
        ThirdwebPaymentsGateway.PayoutInfo[] memory _payouts
    ) internal returns (bytes memory) {
        bytes32 _payoutsHash = _hashPayoutInfo(_payouts);
        ...
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(2, ethSignedMsgHash); // sign with operator private key, i.e. 2
        bytes memory signature = abi.encodePacked(r, s, v);

        return signature;
    }

    function test_transferStart() public {
