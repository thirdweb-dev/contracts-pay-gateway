...
contract PaymentsGatewayPull is EIP712, Ownable, ReentrancyGuard {
    ...
    error PaymentsGatewayInvalidWithdrawal(address caller, address tokenAddress, uint256 tokenAmount);
    ...
    */
    ...
    /// @dev Mapping from pay tokenAddress => payee => feeBalance
    mapping(address => mapping(address => uint256)) public feeBalance;

    /// @dev Mapping from pay tokenAddress => total fee balance for token
    mapping(address => uint256) public feeCorpusForToken;
    ...
    function withdrawTo(
        ...
        uint256 _feeCorpus = feeCorpusForToken[tokenAddress];

        if (_isTokenERC20(tokenAddress)) {
            uint256 tokenBalance = SafeTransferLib.balanceOf(tokenAddress, address(this));
            if (tokenAmount > tokenBalance - _feeCorpus) {
                revert PaymentsGatewayInvalidWithdrawal(msg.sender, tokenAddress, tokenAmount);
            }

            SafeTransferLib.safeTransferFrom(tokenAddress, address(this), receiver, tokenAmount);
        } else {
            if (tokenAmount > address(this).balance - _feeCorpus) {
                revert PaymentsGatewayInvalidWithdrawal(msg.sender, tokenAddress, tokenAmount);
            }
    ...
    function withdrawFee(address tokenAddress, uint256 tokenAmount, address payable receiver) public nonReentrant {
        uint256 _feeBalance = feeBalance[tokenAddress][msg.sender];
        uint256 _feeCorpus = feeCorpusForToken[tokenAddress];

        if (_feeBalance < tokenAmount || _feeCorpus < tokenAmount) {
            revert PaymentsGatewayInvalidWithdrawal(msg.sender, tokenAddress, tokenAmount);
        }

        feeBalance[tokenAddress][msg.sender] -= tokenAmount;
        feeCorpusForToken[tokenAddress] -= tokenAmount;
    ...
    function _distributeFees(
        ...
        for (uint32 payeeIdx = 0; payeeIdx < payouts.length; payeeIdx++) {
            ...
            feeBalance[tokenAddress][payouts[payeeIdx].payoutAddress] += feeAmount;
            feeCorpusForToken[tokenAddress] += feeAmount;

            if (!_isTokenNative(tokenAddress)) {
                SafeTransferLib.safeTransferFrom(tokenAddress, msg.sender, address(this), feeAmount);
            }
